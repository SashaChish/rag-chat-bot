...
vi.mock('@/lib/validators/query.validators', () => ({
  validateQuery: vi.fn(),
}));
...
describe('/api/chat', () => {
  ...
  describe('POST', () => {
    ...
    it('should return 400 when query validation fails', async () => {
      const { validateQuery } = await import('@/lib/validators/query.validators');
      vi.mocked(validateQuery).mockImplementation(() => {
        throw new Error('Invalid query');
      });

      const request = createMockRequest({ message: 'test query' });
