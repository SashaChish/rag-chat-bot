...
vi.mock("@/lib/hooks/useDocumentDownload", () => ({
...
describe("DocumentList", () => {
  beforeEach(() => {
    ...
    global.fetch = vi.fn().mockImplementation((url: string) => {
      ...
      if (url.startsWith("/api/documents/") && !url.includes("/download")) {
        const docId = url.replace("/api/documents/", "");
        const doc = mockDocumentsData.find((d) => d.id === docId);
        return Promise.resolve({
          ok: true,
          json: () =>
            Promise.resolve(
              doc
                ? { ...doc, content: doc.id === "doc1" ? "Sample content for document 1" : null }
                : null,
            ),
        });
      }
  ...
  it("should show loading state initially", async () => {
    ...
    const DocumentList = (
      await import("@/components/DocumentList/DocumentList")
    ).default;
  ...
  it("should open details modal when view details button is clicked", async () => {
    ...
    const detailsButtons = screen.getAllByLabelText(
      /View details for document1\.txt/,
    );
  ...
  it("should display content in preview modal", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText("Document Preview")).toBeInTheDocument();
      expect(
        screen.getByText("Sample content for document 1"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("should show no preview message when content is empty", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText("Document Preview")).toBeInTheDocument();
      expect(
        screen.getByText("No content available for preview"),
      ).toBeInTheDocument();
    });
  });
