...
describe('getSupportedExtensions', () => {
  it('should return all default extensions when no format names provided', () => {
    const result = getSupportedExtensions();
    expect(result).toEqual(expect.arrayContaining(['pdf', 'txt', 'md', 'markdown', 'docx']));
  });

  it('should return extensions for specific format names', () => {
    const result = getSupportedExtensions(['PDF', 'TEXT']);
    expect(result).toEqual(expect.arrayContaining(['pdf', 'txt']));
  });

  it('should handle empty format names array', () => {
    const result = getSupportedExtensions([]);
    expect(result).toEqual(expect.arrayContaining(['pdf', 'txt', 'md', 'markdown', 'docx']));
  });

  it('should handle duplicate format names', () => {
    const result = getSupportedExtensions(['PDF', 'TEXT', 'PDF']);
    expect(result).toEqual(['pdf', 'txt', 'pdf']);
  });

  it('should handle invalid format names', () => {
    const result = getSupportedExtensions(['INVALID', 'PDF']);
...
describe('validateFileExtension', () => {
  ...
  it('should use provided format names for validation', () => {
    expect(validateFileExtension('test.pdf', ['PDF'])).toBe(true);
    expect(validateFileExtension('test.txt', ['PDF'])).toBe(false);
  });
});
...
describe('getUploadErrorMessage', () => {
  ...
  it('should return unknown error message for non-Error objects', () => {
    expect(getUploadErrorMessage('string error')).toBe('Unknown error occurred');
...
describe('getSupportedFormatsText', () => {
  it('should return all formats text when no format names provided', () => {
    const result = getSupportedFormatsText();
    expect(result).toBe('PDF, TEXT, MARKDOWN, DOCX');
  });

  it('should return custom formats text when format names provided', () => {
    const result = getSupportedFormatsText(['PDF', 'TEXT']);
  ...
  it('should join format names with comma and space', () => {
    const result = getSupportedFormatsText(['PDF', 'TEXT', 'DOCX']);
    expect(result).toBe('PDF, TEXT, DOCX');
  });
});
