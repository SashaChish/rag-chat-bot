  ...
  try {
    ...
    if (agentType) {
      try {
        ...
        if (streaming) {
          const response = await agent.chat({ message: query, stream: true });
    ...
    if (chatEngineType) {
      ...
      try {
        const chatEngine = await getChatEngine(index, chatEngineType, chatMessages, sessionKey, systemPrompt || getSystemPrompt());
